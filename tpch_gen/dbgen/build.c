...
extern DSS_HUGE SkewIntCount(long nStream, int skewVal, long n);
...
mk_order(DSS_HUGE index, order_t *o, long upd_num)
{
   ...
   if (scale >= 30000)
		RANDOM64(o->custkey, O_CKEY_MIN, O_CKEY_MAX, O_CKEY_SD, skew, O_CKEY_MAX);
	else
	...
	/* New method for generating skewed data for ORDERS & LINEITEM  */
    ...
    if (scale >= 30000)
        RANDOM64(o->l[lcnt].partkey, L_PKEY_MIN, L_PKEY_MAX, L_PKEY_SD, skew, L_PKEY_MAX);
	else
...
mk_supp(DSS_HUGE index, supplier_t *s)
	...
	*/
	...
	RANDOM(noise, 0, (s->clen - BBB_CMNT_LEN), BBB_JNK_SD,
	 	skew, (S_CMNT_MAX - BBB_CMNT_LEN)+1);
